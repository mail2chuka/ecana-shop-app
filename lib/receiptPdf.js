...
// mirrors PaymentDetailsBox. Drawn high up (right under the header), not at the bottom, since it's
// what someone paying off a debit actually needs to act on. Returns y unchanged if there's nothing set.
export function drawPaymentDetailsBox(pdf, y, org) {
  const rows = [
  ...
  pdf.setDrawColor(210); pdf.setLineWidth(0.75);
  pdf.rect(MARGIN, boxTop, CONTENT_RIGHT - MARGIN, boxHeight);

  let rowY = boxTop + 18;
  pdf.setFont('helvetica', 'bold'); pdf.setFontSize(9); pdf.setTextColor(90);
  pdf.text('Payment Details', MARGIN + 10, rowY);
  rowY += rowHeight;
  ...
  for (const [label, value] of rows) {
    ...
    pdf.text(String(value), CONTENT_RIGHT - 10, rowY, { align: 'right' });
    rowY += rowHeight;
  }
  return boxTop + boxHeight + 22;
}

// Just the invoiceFooter thank-you line now — the bank account moved up into drawPaymentDetailsBox,
// near the top of the receipt instead of at the very bottom.
