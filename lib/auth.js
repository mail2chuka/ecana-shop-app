    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const org = user.organization
          ? await runUnscoped(() => Organization.findById(user.organization).select('name'))
          : null;
        return { id: user._id.toString(), name: user.name, email: user.email, role: user.role, linkedCustomer: user.linkedCustomer ? user.linkedCustomer.toString() : null, organization: user.organization ? user.organization.toString() : null, organizationName: org?.name || null, isPlatformAdmin: !!user.isPlatformAdmin };
      },
    }),
  ],
  callbacks: {
    async jwt({ token, user }) {
      if (user) { token.id = user.id; token.role = user.role; token.linkedCustomer = user.linkedCustomer; token.organization = user.organization; token.organizationName = user.organizationName; token.isPlatformAdmin = user.isPlatformAdmin; }
      return token;
    },
