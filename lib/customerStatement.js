...
export async function buildCustomerStatement(customerId, { startDate, endDate } = {}) {
  ...
  const [sales, payments, standaloneAdjustments] = await Promise.all([
    ...
    CustomerAdjustment.find({ customer: customerId }).sort({ date: -1 }).limit(200),
  ]);

  const ledger = [];
  sales.forEach(s => {
    const qty = s.items.reduce((sum, i) => sum + (i.billQuantity || 0), 0);
    // Per-item breakdown so a multi-product sale can be shown as one line per item (own qty, own
    // real unit price) instead of a single blended row — e.g. summing bags across two different
    // cement brands, or averaging two different prices into one meaningless number.
    const lineItems = s.items.map(i => ({
      name: i.itemType === 'shop' ? i.shopProductName : (i.cementBrandName || `${i.quarryName} ${i.size}`),
      qty: i.billQuantity,
      unit: saleItemUnitLabel(i),
    ...
    ledger.push({
      ...
      saleType: s.saleType,
      ref: s.saleNumber,
      description: `Sales: ${lineItems.map(li => `${li.qty} ${li.unit} ${li.name}`).join(', ')}`,
      items: lineItems,
      qty,
  ...
  standaloneAdjustments.forEach((adj) => {
    const isSurcharge = adj.type === 'surcharge';
    ledger.push({
      date: adj.date,
      ...
      type: adj.type,
      ref: adj.referenceNumber,
      description: `${isSurcharge ? 'Surcharge' : 'Fund'} (not tied to a transaction): ${adj.reason}`,
      qty: null,
      unitPrice: null,
      transport: null,
      debit: isSurcharge ? adj.amount : 0,
