...
async function _h_PUT(request, { params }) {
  try {
    ...
    if (!session || !can(session.user.role, 'suppliers.edit')) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    await dbConnect();
...
export const PUT = withOrg(_h_PUT, 'aggregate');
export const DELETE = withOrg(_h_DELETE, 'aggregate');
