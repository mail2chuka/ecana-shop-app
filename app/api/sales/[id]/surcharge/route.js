...
export async function POST(request, { params }) {
  ...
  try {
    ...
    await mongoSession.withTransaction(async () => {
      ...
      if (sale.saleType === 'shop') throw new ApiError('Surcharges do not apply to shop sales', 400);

      let amount;
      if (method === 'per_unit') {
        const qty = sale.items.reduce((sum, i) => sum + (i.billQuantity || 0), 0);
        amount = Number(perUnitAmount) * qty;
      } else if (method === 'flat_total' || method === 'transport') {
        amount = Number(totalAmount);
      } else {
        throw new ApiError('Invalid surcharge method', 400);
      }
      if (!amount || amount <= 0) throw new ApiError('Enter a valid surcharge amount', 400);
